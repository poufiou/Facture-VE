...
import pandas as pd
import unicodedata, re, os, tempfile
...
def compute_cost(start, active_seconds, kWh_total):
    if kWh_total<=0 or pd.isna(start) or active_seconds<=0: return 0.0, 0.0, 0.0
...
def build_pdf(df, mois_selection, selected_auth):
    ...
    if None in [c_start, c_end, c_energy, c_auth]:
        return None, "Colonnes manquantes dans le CSV."
...
if csv_file is not None:
    ...
    if st.button("📄 Générer la facture PDF"):
        output, err = build_pdf(df, mois_selection, selected_auth if selected_auth!="Tous" else "")
        if err:
            st.error("⚠️ " + err)
        else:
